...
class WPP_MailChimp {
	 ...
	 * @var string
	 */
	protected $transient_name = 'wpb_mailchimp_lists';
	...
	public function get_lists( $force_refresh = false, $force_fallback = false ) {
		...
		if( ! is_array( $cached_lists ) ) {
			...
			// make api request for lists
                                $wpb_object = new WPP_CLASS();	
                                $api = $wpb_object->wpp_mc_get_api();
	...
	public function get_subscriber_count( $list_ids ) {
		...
		$list_counts = get_transient( 'wpb_list_counts' );
		...
		if( false === $list_counts ) {
			...
			if ( is_array( $lists ) ) {
				...
				$transient_lifetime = apply_filters( 'wpb_lists_count_cache_time', 1200 ); // 20 mins by default

				set_transient( 'wpb_list_counts', $list_counts, $transient_lifetime );
				set_transient( 'wpb_list_counts_fallback', $list_counts, 86400 ); // 1 day
			} else {
				// use fallback transient
				$list_counts = get_transient( 'wpb_list_counts_fallback' );
				if ( false === $list_counts ) {
					return 0;
				}
			}
		...
		return apply_filters( 'wpb_subscriber_count', $count );
	}
